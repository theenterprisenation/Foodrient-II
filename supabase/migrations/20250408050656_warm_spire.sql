/*
  ...
  1. Changes
    - Add fixed search path to all remaining functions
...
CREATE TRIGGER check_advertisement_expiry
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.check_advertisement_expiry();
...
CREATE TRIGGER order_points_calculation
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.calculate_referral_points();

CREATE TRIGGER update_vendor_metrics_trigger
  AFTER UPDATE ON orders
  FOR EACH ROW
  EXECUTE FUNCTION public.update_vendor_metrics();

CREATE TRIGGER update_group_buy_metrics_trigger
  AFTER UPDATE ON group_buys
  FOR EACH ROW
  EXECUTE FUNCTION public.update_group_buy_metrics();
